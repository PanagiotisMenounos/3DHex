...
class PinsWindow(QtWidgets.QMainWindow, Ui_pinswindow):
    ...
    def initiate(self, *args, obj=None, **kwargs):
        ...
        getattr(self, "pin{}".format(self.window.pin_conf)).setStyleSheet("QPushButton {\n"
    ...
    def select_pin(self,pin):
        if pin==70:
            getattr(self.window, "Pin_Button_{}".format(self.window.pin_button)).setText('N')
        else:
            getattr(self.window, "Pin_Button_{}".format(self.window.pin_button)).setText(str(pin))
        self.close()